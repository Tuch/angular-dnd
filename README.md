...
Параметр (задаются в виде объекта, callbacks задаются в кавычках):
- onstart() - функция, срабатывающая в начале изменения размеров элемента. В отличии от dragable, resizable и rotatable директив, где start событие срабатывет после начала движения манипултора (mousemove, touchmove) здесь началом считаются события mousedown/touchstart
- ondrag() - функция, срабатывающая при изменении размеров элемента 
- onend() - функция, срабатывающая в конце изменения размеров элемента 
