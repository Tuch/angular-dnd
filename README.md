...
### dnd-none-draggable
Атрибут используется для того, что бы дочерние ноды dnd-draggable элемента можно было помечать как "не передвигаемые" (см.пример 2 sortable директивы - поле для ввода текста)
